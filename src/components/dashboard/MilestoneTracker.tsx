...
const MilestoneTracker = () => {
  ...
  return <motion.div initial={{
    ...
    delay: 0.2
      ...
      {/* Milestones */}
      <div ref={containerRef} className="max-h-[350px] sm:max-h-[400px] overflow-y-auto pr-2 space-y-2 sm:space-y-3 scrollbar-hide">
        {milestones.map((milestone, index) => {
        const styles = getStatusStyles(milestone.status);
        const isExpanded = expandedId === milestone.id;
        const isInProgress = milestone.status === "in-progress";
        return <motion.div key={milestone.id} ref={isInProgress ? inProgressRef : null} initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.1 + index * 0.1
        }} className={`rounded-xl md:rounded-2xl border ${styles.border} transition-all duration-300 overflow-hidden ${isInProgress ? "shadow-[0_0_30px_rgba(212,175,55,0.15)] bg-gradient-to-r from-champagne-gold/5 to-transparent" : "bg-white/50"}`}>
              <button onClick={() => toggleExpand(milestone.id)} className="w-full flex items-center gap-3 sm:gap-4 p-3 sm:p-4 text-left hover:bg-white/50 transition-colors">
                {/* Status Icon */}
              ...
              scale: 1.1
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isInProgress && milestone.progress && <div className="mt-2 w-full max-w-[200px]">
              ...
              duration: 0.3
                    ...
                    <div className="px-3 sm:px-4 pb-3 sm:pb-4 pt-0">
                      <div className="pl-11 sm:pl-14">
                        ...
                        {milestone.photos.length > 0 ? <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 scrollbar-hide">
                            {milestone.photos.map((photo, photoIndex) => <motion.div key={photo.id} initial={{
                      opacity: 0,
                      scale: 0.8
                    }} animate={{
                      opacity: 1,
                      ...
                      delay: photoIndex * 0.1
                          ...
                          </div> : <p className="text-xs text-charcoal/40 italic">
      ...
      delay: 0.6
        ...
        <Link to="/progress" className="group flex items-center gap-2 text-xs uppercase tracking-[0.15em] text-champagne-gold hover:text-charcoal transition-colors duration-300">
