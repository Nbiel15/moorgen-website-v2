...
const scenes = [
  ...
  { id: "relax", label: "Relax", icon: Sofa },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div className="space-y-8">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {scenes.map((scene, index) => (
                  ...
                  >
                    <motion.div
                      animate={activeScene === scene.id ? { rotate: [0, 10, -10, 0] } : {}}
                      transition={{ duration: 0.5 }}
                    >
                      <scene.icon
                        className={cn("w-6 h-6", activeScene === scene.id && "text-accent")}
                        strokeWidth={1.5}
                      />
                    </motion.div>
                    <span className={cn(
                      "text-xs font-body tracking-[0.1em]",
                      activeScene === scene.id && "text-accent"
                    )}>
                      {scene.label}
                    </span>
                    {activeScene === scene.id && (
            ...
            >
              ...
              <div className="flex gap-2">
                {["Warm", "Neutral", "Cool"].map((temp, index) => (
                  <motion.button
                    key={temp}
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
