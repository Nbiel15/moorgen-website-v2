...
const EngineerChat = () => {
  ...
  const quickResponses = ["Request Latest Photos", "Check Deadline Status", "Book Site Visit"];
      ...
      <header className="flex-shrink-0 bg-background border-b border-border px-3 sm:px-4 py-2.5 sm:py-3">
        <div className="flex items-center justify-between max-w-2xl mx-auto">
          <div className="flex items-center gap-2 sm:gap-3">
            <motion.button onClick={() => navigate(-1)} whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
              ...
              <div className={`max-w-[80%] sm:max-w-[75%] px-3 sm:px-4 py-2 sm:py-3 shadow-sm ${message.sender === "engineer" ? "bg-champagne-gold/15 border border-champagne-gold/30 text-foreground rounded-2xl rounded-bl-md" : "bg-background border-2 border-champagne-gold/40 text-foreground rounded-2xl rounded-br-md"}`}>
                ...
                <div className={`flex items-center gap-1 mt-1 sm:mt-1.5 ${message.sender === "owner" ? "justify-end" : ""}`}>
                  ...
                  {message.sender === "owner" && message.status && <span className="text-champagne-gold">
                      {message.status === "read" ? <CheckCheck className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> : <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5" />}
                    </span>}
                </div>
              </div>

              {message.sender === "owner" && <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br from-champagne-gold to-champagne-gold/80 flex items-center justify-center text-background text-[10px] sm:text-xs font-medium flex-shrink-0">
                  A
                </div>}
            </motion.div>)}
      ...
      <div className="flex-shrink-0 bg-background border-t border-border px-3 sm:px-4 py-2.5 sm:py-3 pb-safe">
        <div className="max-w-2xl mx-auto space-y-2 sm:space-y-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 scrollbar-hide -mx-1 px-1">
            {quickResponses.map(response => <motion.button key={response} onClick={() => sendMessage(response)} whileHover={{
            scale: 1.02
          }} whileTap={{
            scale: 0.98
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={handleKeyPress} placeholder="Type a message..." className="flex-1 bg-muted/50 border-0 rounded-full text-sm h-9 sm:h-11 px-3 sm:px-4 font-body" />
            <motion.button onClick={handleSend} whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }} disabled={!chatInput.trim()} className="p-2 sm:p-2.5 bg-champagne-gold text-background rounded-full hover:bg-champagne-gold/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0">
