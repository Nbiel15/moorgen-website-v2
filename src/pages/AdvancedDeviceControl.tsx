      ...
      <main className="relative mx-auto w-full max-w-7xl px-4 py-6 md:px-8 pb-24 lg:pb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          duration: 0.6
            ...
            type: "spring",
            stiffness: 100
          }} className={cn("relative w-64 h-64 md:w-80 md:h-80 transition-all duration-500 rounded-full", !isPowered && "opacity-50")}>
              {/* Dynamic Background Gradient - Only on dial */}
              <div className={cn("absolute -inset-8 rounded-full bg-gradient-to-br transition-all duration-1000 blur-2xl opacity-50", getTemperatureGradient())} />
              {/* Background Track - 270 degree arc */}
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="44" fill="none" stroke="hsl(0, 0%, 90%)" strokeWidth="6" strokeLinecap="round" strokeDasharray="207.35 276.46" transform="rotate(-225, 50, 50)" />
              </svg>
              
              {/* Active Arc with draw animation */}
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
                <motion.circle cx="50" cy="50" r="44" fill="none" stroke={isPowered ? "url(#champagneGoldGradient)" : "hsl(0, 0%, 70%)"} strokeWidth="6" strokeLinecap="round" initial={{
                strokeDasharray: "0 276.46"
              }} animate={{
              ...
              <motion.div className="absolute inset-4 rounded-full bg-card border border-border flex items-center justify-center" style={{
              rotate: dialRotationSpring
            }}>
                <motion.div animate={{
                scale: isPowered ? [1, 1.2, 1] : 1,
                boxShadow: isPowered ? ["0 0 10px rgba(212,175,55,0.5)", "0 0 20px rgba(212,175,55,0.8)", "0 0 10px rgba(212,175,55,0.5)"] : "none"
              }} transition={{
                duration: 2,
                repeat: Infinity
              }} className={cn("absolute top-4 left-1/2 -translate-x-1/2 w-4 h-4 rounded-full transition-all duration-300", isPowered ? "bg-accent" : "bg-muted-foreground/50")} />
              </motion.div>
              ...
              <div className="absolute inset-8 rounded-full bg-card flex flex-col items-center justify-center shadow-inner">
                <motion.button whileHover={{
                scale: 1.2,
                backgroundColor: "hsla(43, 76%, 52%, 0.1)"
              }} whileTap={{
                scale: 0.9
              }} onClick={() => setTemperature(prev => Math.min(maxTemp, prev + 1))} className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all mb-1", isPowered ? "text-muted-foreground hover:text-accent cursor-pointer" : "text-muted-foreground/30 cursor-not-allowed")} disabled={!isPowered || temperature >= maxTemp}>
                  <ChevronUp className="w-6 h-6" strokeWidth={1.5} />
                </motion.button>
                
                {/* Animated Temperature Display */}
                <motion.span key={displayTemp} initial={{
                y: tempAnimating ? -10 : 0,
                opacity: tempAnimating ? 0 : 1
              }} animate={{
                y: 0,
                opacity: 1
              }} className={cn("font-heading text-5xl md:text-6xl transition-colors duration-300", isPowered ? "text-foreground" : "text-muted-foreground")}>
                  {displayTemp}°C
                </motion.span>
                
                <motion.span animate={{
                opacity: isPowered ? 1 : 0.5
              }} className={cn("text-[10px] font-body tracking-[0.2em] uppercase mt-1 transition-colors duration-300", isPowered ? "text-accent" : "text-muted-foreground/50")}>
                  {isPowered ? temperature < 22 ? "Cooler" : temperature > 26 ? "Warmer" : "Comfort" : "Off"}
                </motion.span>
                ...
                scale: 0.9
              }} onClick={() => setTemperature(prev => Math.max(minTemp, prev - 1))} className={cn("w-10 h-10 rounded-full flex items-center justify-center transition-all mt-1", isPowered ? "text-muted-foreground hover:text-accent cursor-pointer" : "text-muted-foreground/30 cursor-not-allowed")} disabled={!isPowered || temperature <= minTemp}>
          ...
          <div className="space-y-8">
            ...
            delay: 0.5
              ...
              <div className="grid grid-cols-3 gap-3">
                {modes.map((mode, index) => <motion.button key={mode.id} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
                ...
                scale: 0.95,
                rotateY: 10
              }} onClick={() => handleModeChange(mode.id)} className={cn("relative flex flex-col items-center gap-3 p-5 rounded-3xl transition-all duration-300 border hover-lift overflow-hidden", activeMode === mode.id ? "bg-primary text-primary-foreground border-primary shadow-lg" : "bg-card border-border text-muted-foreground hover:border-foreground/20")}>
                    <motion.div animate={activeMode === mode.id ? {
                  rotate: [0, 10, -10, 0]
                } : {}} transition={{
                  duration: 0.5
                }}>
                    ...
                    {activeMode === mode.id && <motion.div layoutId="modeGlow" className="absolute inset-0 bg-accent/10 rounded-3xl" initial={{
                  opacity: 0
                }} animate={{
                  opacity: 1
                }} exit={{
            ...
            delay: 0.7
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                scale: 0.98
              }} onClick={() => handleRoomChange(room.id)} className={cn("relative flex items-center justify-between p-5 rounded-3xl transition-all duration-300 border hover-lift", activeRoom === room.id ? "bg-accent/10 text-foreground border-accent/30" : "bg-card border-border text-foreground hover:border-foreground/20")}>
            ...
            delay: 0.9
              ...
              <div className="flex gap-2">
                {["Auto", "Low", "Med", "High"].map((speed, index) => <motion.button key={speed} initial={{
                opacity: 0,
                y: 10
              }} animate={{
                opacity: 1,
                ...
                scale: 0.95
              }} onClick={() => handleFanSpeedChange(speed)} className={cn("relative flex-1 py-3 rounded-2xl text-sm font-body tracking-[0.1em] transition-all border overflow-hidden", fanSpeed === speed ? "bg-primary text-accent border-primary shadow-lg" : "bg-card border-border text-muted-foreground hover:border-foreground/20")}>
                    {speed}
                    {fanSpeed === speed && <motion.div className="absolute inset-0 bg-gradient-to-r from-transparent via-accent/20 to-transparent" animate={{
                  x: ["-100%", "100%"]
                }} transition={{
                  duration: 1.5,
                  repeat: Infinity,
