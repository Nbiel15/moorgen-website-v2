...
const manuals = [
  { name: "Milan Series Touch Panel Guide", size: "2.4 MB", type: "PDF" },
  { name: "HVAC Integration Manual", size: "1.8 MB", type: "PDF" },
  { name: "Lighting Scene Programming", size: "3.1 MB", type: "PDF" },
  { name: "Security System Quick Start", size: "1.2 MB", type: "PDF" }
];
...
const SupportManagerPanel = () => {
  ...
  return (
    <div className="hidden lg:block w-80 xl:w-96 border-l border-border/50 bg-gradient-to-b from-background to-muted/20 p-6 overflow-y-auto">
      <motion.div 
      ...
      >
        ...
        <div className="bg-foreground rounded-2xl p-5 text-background">
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-champagne-gold text-foreground font-body font-medium text-sm hover:bg-champagne-gold/90 transition-colors"
          >
...
const Support = () => {
  return (
    <DashboardLayout showRightPanel rightPanel={<SupportManagerPanel />}>
              ...
              >
                <div className="relative">
                  ...
                  <Input 
                    ...
                    className="pl-11 pr-4 py-6 rounded-xl border-border/50 bg-background/80 backdrop-blur-sm font-body text-sm focus:border-champagne-gold/50 focus:ring-champagne-gold/20"
                  />
                </div>
              </motion.div>
            </div>
  ...
  return (
    <div className="bg-foreground rounded-2xl p-5 text-background">
      ...
      <div className="flex items-center gap-4">
        ...
        <Button
          onClick={() => navigate("/engineer-chat")}
          className="bg-champagne-gold hover:bg-champagne-gold/90 text-foreground font-body text-sm"
        >
...
const MobileDocumentation = () => {
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-2xl p-5 border border-border/50">
      <h3 className="font-heading text-lg text-foreground mb-4">Quick Manuals</h3>
      
      <div className="grid grid-cols-2 gap-3">
        {manuals.slice(0, 4).map((manual, index) => (
          <motion.button
            key={index}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/50 hover:bg-champagne-gold/10 border border-transparent hover:border-champagne-gold/20 transition-all text-center"
          >
            ...
            <p className="text-[11px] font-body text-foreground line-clamp-2">{manual.name}</p>
            <p className="text-[10px] text-muted-foreground font-body">{manual.size}</p>
          </motion.button>
        ))}
      </div>
    </div>
