...
import { SunDim, Lock, LockOpen, Thermometer, Blinds, Zap, Activity, Wifi, ChevronRight, Sparkles, Power, Shield, Home, MoreHorizontal, TrendingUp, Battery, Signal } from "lucide-react";
...
const Controls = () => {
  ...
  return <DashboardLayout>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 py-6 md:px-8 pb-24 lg:pb-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 mb-8 scrollbar-hide animate-fade-in" style={{
        animationDelay: "0.2s"
      }}>
          {roomTabs.map((room, index) => <button key={room.id} onClick={() => setActiveRoom(room.id)} className={cn("relative flex items-center gap-2 px-4 md:px-6 py-2.5 md:py-3 rounded-full whitespace-nowrap transition-all duration-500 font-body text-sm", activeRoom === room.id ? "bg-gradient-to-r from-accent via-accent to-champagne-light text-accent-foreground shadow-lg shadow-accent/20 scale-105" : "bg-card border border-border text-muted-foreground hover:border-accent/30 hover:bg-muted/50 hover:text-foreground")} style={{
          animationDelay: `${0.2 + index * 0.05}s`
        }}>
              {room.icon && <room.icon className="w-4 h-4" />}
              <span className="font-medium">{room.label}</span>
              {room.count && <span className={cn("text-xs px-1.5 py-0.5 rounded-full", activeRoom === room.id ? "bg-accent-foreground/20 text-accent-foreground" : "bg-muted text-muted-foreground")}>
                  {room.count}
                </span>}
            </button>)}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:gap-6">
          {devices.map((device, index) => <div key={device.id} className={cn("group relative rounded-3xl overflow-hidden cursor-pointer animate-fade-in", "transition-all duration-700 ease-out", "hover:scale-[1.02] hover:-translate-y-2", device.isOn ? "bg-gradient-to-br from-primary via-primary/95 to-primary/90 text-primary-foreground shadow-2xl shadow-primary/30" : "bg-card border border-border hover:border-accent/40 hover:shadow-2xl hover:shadow-accent/10")} style={{
          animationDelay: `${0.3 + index * 0.1}s`
              ...
              <div className={cn("absolute -top-16 -right-16 w-32 h-32 rounded-full blur-3xl transition-all duration-700", device.isOn ? "bg-accent/40 group-hover:bg-accent/60" : "bg-transparent group-hover:bg-accent/10")} />

              <div className="relative p-5 md:p-6">
                {/* Top Section: Icon + Connection Status */}
                <div className="flex items-start justify-between mb-6">
                  {/* Device Icon Container */}
                  <div className={cn("relative w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center", "transition-all duration-500", device.isOn ? "bg-accent/20 shadow-[0_0_40px_rgba(212,175,55,0.5)] ring-2 ring-accent/30" : "bg-muted/50 group-hover:bg-accent/10 group-hover:shadow-xl group-hover:ring-1 group-hover:ring-accent/20")}>
                    {/* Active Sparkle */}
                    {device.isOn && <Sparkles className="absolute -top-1.5 -right-1.5 w-4 h-4 text-accent animate-pulse" />}
                    
                    {device.id === "lock" ? device.isOn ? <Lock className="w-7 h-7 md:w-8 md:h-8 text-accent transition-all duration-500 group-hover:scale-110" strokeWidth={1.5} /> : <LockOpen className="w-7 h-7 md:w-8 md:h-8 text-foreground transition-all duration-500 group-hover:scale-110 group-hover:text-accent" strokeWidth={1.5} /> : <device.icon className={cn("w-7 h-7 md:w-8 md:h-8 transition-all duration-500 group-hover:scale-110", device.isOn ? "text-accent" : "text-foreground group-hover:text-accent")} strokeWidth={1.5} />}
                  </div>

                  {/* Connection Indicator */}
                  <div className={cn("flex items-center gap-1.5 px-2.5 py-1.5 rounded-full", "transition-all duration-300 backdrop-blur-sm", device.isOn ? "bg-green-500/20 text-green-400 ring-1 ring-green-500/30" : "bg-muted/80 text-muted-foreground group-hover:bg-accent/10 group-hover:text-accent group-hover:ring-1 group-hover:ring-accent/20")}>
                    <span className={cn("w-2 h-2 rounded-full", device.isOn ? "bg-green-400 animate-pulse" : "bg-muted-foreground")} />
                ...
                <div className="space-y-1.5 mb-4">
                  <p className={cn("text-[11px] tracking-[0.25em] uppercase font-body", device.isOn ? "text-accent" : "text-muted-foreground")}>
                    {device.series}
                  </p>
                ...
                {/* Bottom Controls */}
                <div className={cn("flex items-center justify-between pt-4 border-t", device.isOn ? "border-primary-foreground/10" : "border-border")}>
                  <div className="flex items-center gap-3">
                    <Switch checked={device.isOn} onCheckedChange={() => toggleDevice(device.id)} onClick={e => e.stopPropagation()} className={cn("transition-all duration-300", device.isOn && "data-[state=checked]:bg-accent shadow-[0_0_20px_rgba(212,175,55,0.4)]")} />
                    <span className={cn("text-xs font-body font-medium tracking-[0.2em] uppercase", device.isOn ? "text-accent" : "text-muted-foreground")}>
        ...
        animationDelay: "0.7s"
          ...
          <button className="w-full group relative overflow-hidden rounded-3xl p-6 border-2 border-dashed border-border hover:border-accent/50 transition-all duration-500 hover:bg-accent/5">
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-accent/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 translate-x-[-100%] group-hover:translate-x-[100%]" style={{
