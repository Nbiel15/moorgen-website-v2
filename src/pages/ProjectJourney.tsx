    ...
    switch (status) {
      ...
      case "in-progress":
        return {
          ...
          icon: <motion.div animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="w-2 h-2 bg-white rounded-full" />,
      ...
      <div className="min-h-screen bg-background">
          ...
          <div className="relative mx-auto w-full max-w-7xl px-4 py-6 md:px-8 md:py-10">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 md:gap-6">
              ...
              <motion.div 
                ...
                className="flex items-center gap-3 px-4 py-2 rounded-2xl bg-card border border-border/50 shadow-sm"
              >
                <div className="relative w-12 h-12">
                  ...
                  <span className="absolute inset-0 flex items-center justify-center text-xs font-semibold">{totalProgress}%</span>
                </div>
                <div>
                  <p className="text-sm font-medium text-foreground">Overall Progress</p>
                  <p className="text-[10px] text-champagne-gold">On Track</p>
                </div>
              </motion.div>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`flex-1 ${isLast ? 'pb-0' : 'pb-2'}`}>
                        <div className={`rounded-xl md:rounded-2xl border transition-all duration-300 overflow-hidden ${
                          isInProgress 
                            ? "border-champagne-gold bg-gradient-to-r from-champagne-gold/5 to-transparent shadow-[0_0_30px_rgba(212,175,55,0.1)]" 
                            : "border-border hover:border-muted-foreground/30 hover:bg-muted/20"
                        }`}>
                          {/* Header */}
                          <button 
                            onClick={() => togglePhase(phase.id)}
                            className="w-full p-3 sm:p-4 text-left flex items-start justify-between"
                          >
                            <div className="flex-1">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {phase.status === "completed" && (
                                  <span className={`px-2 py-0.5 text-[9px] sm:text-[10px] font-semibold uppercase rounded-full ${styles.badge}`}>
                                    Done
                                  </span>
                                )}
                              </div>
              ...
              <div className="bg-muted/30 rounded-xl p-3 sm:p-4 mb-4">
                ...
                <div className="grid grid-cols-7 gap-1 text-center mb-2">
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                    <span key={i} className="text-[9px] sm:text-[10px] text-muted-foreground">{d}</span>
                  ))}
                </div>
                  ...
                  {[...Array(31)].map((_, i) => {
                    ...
                    return (
                      <motion.span 
                        key={i}
                        whileHover={{ scale: 1.1 }}
                        className={`text-[10px] sm:text-xs py-1 sm:py-1.5 rounded-lg cursor-pointer transition-colors ${
                          isHighlight 
                            ? "bg-champagne-gold text-white font-semibold shadow-lg" 
                            : isPast 
                  ...
                  <tbody>
                    {evidenceData.map((item, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 sm:py-4 px-2">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-semibold uppercase ${
                        ...
                        <td className="py-3 sm:py-4 px-2 text-right">
                          <Button 
                            variant="ghost" 
                            size="sm" 
                            className="text-[10px] sm:text-xs h-7 sm:h-8 text-champagne-gold hover:text-champagne-gold hover:bg-champagne-gold/10"
                            onClick={() => {
            ...
            >
              ...
              <div className="flex items-center gap-2 pt-3 border-t border-border">
                ...
                <Input
                  ...
                  className="flex-1 bg-muted/50 border-0 rounded-xl text-xs sm:text-sm h-8 sm:h-9"
                />
                <motion.button 
        ...
        <motion.button
          ...
          className={`fixed bottom-24 lg:bottom-8 right-4 sm:right-6 z-50 flex items-center gap-2 px-4 py-3 bg-charcoal text-white rounded-full shadow-xl ${
            showFloatingChat ? '' : 'pointer-events-none'
          }`}
        >
